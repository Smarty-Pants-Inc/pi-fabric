...
Each Herdr tab is labelled with the child's name and the first 12 characters of its run ID. When a `layout.apply` reply is dropped, Fabric adopts the one tab with that label instead of launching again. Before a launch, it closes any tab that an earlier attempt of the same run left behind. A dropped `pane.get` does not end a run. A Herdr server whose socket is gone, as in a live handoff, counts as a lost pane only after 5 minutes. Only `pane_not_found` from a reachable server ends a run at once. Before Fabric relaunches a run, it stops the previous worker, and it relaunches only when that worker is gone for certain. Otherwise the run fails instead of running twice.

Every launch takes a slot from a budget of 20 `layout.apply` calls per minute for each Herdr server (smarty-dev#266). The ledger is a private directory next to the server's real socket (`<socket>.pi-fabric-spawns`, mode 0700). It is shared by every Fabric process of that user, whatever its `TMPDIR`. When a minute's slots are taken, a launch waits for the next minute, for at most 2 minutes. It then fails with an error that names transport `"process"`. Closing the agent manager ends the wait. A ledger that Fabric cannot use safely (not a directory, not owned by the user, or writable by others) turns the budget off, with one warning. Fabric builds without this budget ignore the ledger, so the budget is advisory until every Fabric process that launches into the server runs a build that has it. Actors and background agents should use `transport: "process"`.
